...
def naked_twins(values):
    ...
    for box in twins_boxes:
        ...
        for unit in units[box]:
            keys = [b for b in unit if values[b] == digits]
            if len(keys) != 2:
                continue
            boxes = [b for b in unit if b not in keys]
            for b in boxes:
                for digit in digits:
...
def reduce_puzzle(values):
    """
    Return the sudoku values if the sudoku is solved
    Args:
...
if __name__ == '__main__':
    ...
    try:
        ...
        logger.warning('We could not visualize your board due to a pygame issue. Not a problem! It is not a requirement.')
